...
# define GTK_IS_SDL_CLASS(klass)	(GTK_CHECK_CLASS_TYPE ((klass), GTK_TYPE_SDL))
    ...
    GtkWidget *gtk_sdl_new (gint width, gint height, gint bpp, Uint32 flags,SDL_Surface * screen);//Modification de la fonction
    void       gtk_sdl_size		(GtkSDL      *sdl,
                                 gint                 width,
                                 gint                 height);
    
    /* On met le prototype de la fonction d'update d'un widget sdl accéssible via gtksdl.h,
     on enleve donc le mot clé static pour pouvoir l'utiliser*/
